...
def main():
    ...
    for mdx, pretrained_model_path in enumerate(args.pretrained_model_paths):
        ...
        scores_with_jailbreak = get_following_indices(
            model_name, config=args.config, use_jailbreak_prompt=True, use_harmless=False, return_only_scores=True)
        scores_with_jailbreak_harmless = get_following_indices(
            model_name, config=args.config, use_jailbreak_prompt=True, use_harmless=True, return_only_scores=True)
        ...
        scores_with_jailbreak = torch.tensor(scores_with_jailbreak, device='cuda', dtype=torch.float)
