...
from generate import process_jailbreak_prompt_as_word_embedding
...
def prepend_sys_prompt(sentence, prompt_length):
    ...
    messages = [{'role': 'system', 'content': ''.join([f'<jailbreak_prompt_{i}>' for i in range(prompt_length)])}] + messages
    return messages
...
def main():
    ...
    parser.add_argument("--jailbreak_prompt_path", type=str, default="./trained_prompts_attack")
    ...
    parser.add_argument("--output_path", type=str, default='./hidden_states_with_jailbreak')
    ...
    with safe_open(f'{args.jailbreak_prompt_path}/{model_name}/type.all_length.20.safetensors', framework='pt') as f:
        jailbreak_prompt = f.get_tensor('jailbreak_prompt')
    toker, new_input_embeddings = process_jailbreak_prompt_as_word_embedding(model, toker, jailbreak_prompt)
    ...
    all_messages = [prepend_sys_prompt(l, jailbreak_prompt.size(0)) for l in all_queries]
