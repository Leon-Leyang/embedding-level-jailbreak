...
def main():
    ...
    parser.add_argument("--output_path", type=str, default='./trained_prompts_ortho')
    ...
    for epoch_idx, batch_messages, batch_labels in get_shuffled_messages_and_labels(all_messages, labels, seed=seed):
        ...
        ortho_loss = torch.mean(torch.norm(
                diff -
                torch.matmul(diff, r_weight.transpose(0, 1)) * r_weight / torch.norm(r_weight)**2 -
                torch.matmul(diff, h_weight.transpose(0, 1)) * h_weight / torch.norm(h_weight)**2,
                p=1, dim=1))

        # ortho_loss = torch.mean(torch.norm(new_pca_transformed - base_pca_transformed -
        #                                    torch.matmul(new_pca_transformed - base_pca_transformed, r_weight.transpose(0, 1)) * () -
        #                                    torch.matmul(new_pca_transformed - base_pca_transformed, h_weight.transpose(0, 1)), dim=1))
        ...
        if step % 10 == 0:
            logging.info(f'Step {step}, refusal_loss {refusal_loss.cpu().item()}, harmfulness_loss {harmfulness_loss.cpu().item()}, norm_loss {norm_loss.cpu().item()}, ortho_loss {ortho_loss.cpu().item()}')
